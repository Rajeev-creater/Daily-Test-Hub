...
// Add daily auto-update at 12 AM
function setupDailyReset() {
  ...
  if (lastReset !== today) {
    ...
    localStorage.removeItem(`study_time_${today}`);
    localStorage.removeItem(`user_motivations_${today}`);
    localStorage.removeItem(`motivation_${today}`);
  }
}
...
document.getElementById("backToSubjectBtn").onclick = () => {
  currentSubject = "";
  currentChapter = "";
...
function setupSidebar() {
  ...
  shareBtn.onclick = () => {
    const url = "https://daily-test-hub.onrender.com/";
    const text = "🎓 Rajeev's Study Hub - Free MCQ Tests for Class 9-12\n💡 AI Doubt Solver (Mahadev) | Daily Tests | Study Tracking\n🚀 Perfect for exam preparation!\n\nWebsite: " + url;
    
    if (navigator.share) {
      navigator.share({ title: "Rajeev Study Hub", text, url });
    } else {
      // Copy to clipboard
      navigator.clipboard.writeText(url).then(() => {
        alert("✅ Website URL copied!\n\n" + url + "\n\nShare this link with your friends! 🎉");
      }).catch(() => {
        alert("📱 Website: " + url + "\n\n💬 Share: 'Check Rajeev Study Hub - Free tests for Class 9-12 with AI tutor!'");
      });
    }
  };
  
  rateBtn.onclick = () => {
    window.location.href = "https://daily-test-hub.onrender.com";
  };
}
...
function setupAIWidget() {
  ...
  document.getElementById("askBtn").onclick = async () => {
    ...
    try {
      ...
      renderAIChat();
      input.placeholder = "Type your next doubt...";
    } catch (e) {
      console.error(e);
      aiHistory.push({ role: "assistant", text: "Sorry, could not get response. Try again!" });
      renderAIChat();
...
function renderAIChat() {
  ...
  if (aiHistory.length === 0) {
    replyDiv.innerHTML = `<div style="padding: 10px; color: #999; text-align: center;">No chat history</div>`;
    return;
  }
  replyDiv.innerHTML = `
    <div class="ai-history">
      <div style="color: #2ecc71; font-weight: bold; padding: 8px 0; border-bottom: 1px solid #3d3e5f; margin-bottom: 8px;">Chat History</div>
      ${aiHistory.map((msg, idx) => `
        <div class="ai-message ${msg.role}">
          <div class="message-text">${msg.text}</div>
          ${msg.role === 'user' ? `<button onclick="deleteAIMessage(${idx})" class="delete-btn" title="Delete">×</button>` : ''}
        </div>
      `).join('')}
    </div>
  `;
